...
Based on their mood selection, generate **3 distinct style previews** in a **single HTML file with tabs**. Each preview should be a single title slide showing:
...
**Named Style Library (use these memorable names):**

| Mood | Named Styles |
|------|--------------|
| Impressed/Confident | "Executive", "Corporate", "Minimal" |
| Excited/Energized | "NeonCyber", "BoldGradient", "Kinetic" |
| Calm/Focused | "PaperInk", "DevTerminal", "SwissMinimal" |
| Inspired/Moved | "Cinematic", "Editorial", "Atmospheric" |

These names can be referenced by users in future sessions (e.g., "use the DevTerminal style").
...
### Step 2.3: Present Previews (Tabbed Single File)

Create a **single HTML file** with all 3 style previews accessible via clickable tabs:

```
.claude-design/slide-previews/
└── style-picker.html   # Single file with tabbed previews
```

The tabbed preview file should:
- Show 3 tabs at the top with the style names
- Clicking a tab switches to that style's preview (no page reload)
- Each preview is a title slide demonstrating the aesthetic
- Animations replay when switching tabs
- Include a "Select This Style" button on each preview

**Example tab structure:**
```html
<nav class="style-tabs">
    <button class="tab active" data-style="style-a">StyleNameA</button>
    <button class="tab" data-style="style-b">StyleNameB</button>
    <button class="tab" data-style="style-c">StyleNameC</button>
</nav>
<div class="preview-container">
    <div class="preview active" id="style-a">...</div>
    <div class="preview" id="style-b">...</div>
    <div class="preview" id="style-c">...</div>
</div>
```

Present to user:
```
I've created a style picker with 3 options:

**[StyleNameA]** — [1 sentence description]
**[StyleNameB]** — [1 sentence description]
**[StyleNameC]** — [1 sentence description]

Open this file and click the tabs to compare:
- .claude-design/slide-previews/style-picker.html

Tell me which style you prefer (you can reference it by name in future sessions too).
```
...
- Options:
  - "[StyleNameA]" — [Brief description]
  - "[StyleNameB]" — [Brief description]
  - "[StyleNameC]" — [Brief description]
  - "Mix elements" — Combine aspects from different styles

If "Mix elements", ask for specifics.

**Shortcut:** If the user requests a named style from the library (e.g., "use DevTerminal"), skip the style picker phase entirely and proceed directly to generation.
...
2. **Ask about save location**
   Use AskUserQuestion to ask where to save the presentation:
   - Header: "Location"
   - Question: "Where should I save this presentation?"
   - Options:
     - "tutorials folder" — Save to ~/claude/tutorials/
     - "Current directory" — Save in the working directory
     - "Custom location" — Specify a different path

   Move the file to the chosen location.

3. **Open the presentation**
   - Use `start "" "[filepath]"` (Windows) or `open [filepath]` (macOS) to launch in browser
